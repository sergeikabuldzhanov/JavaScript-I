...
//Stretch Challenge
console.log('Now with the map method');
carModels = [];
carModelsSorted= [];
carModels = inventory.map((elem) => elem.car_model);
carModelsSorted = carModels.sort();
console.log(carModelsSorted);
...
//Streth
console.log('Now with map method!');
carYears = [];
carYears = inventory.map((elem) => elem.car_year);
console.log(carYears);
...
for(let i=0; i<carYears.length;i++){
  if(carYears[i]<2000){
    oldCars.push([inventory[i].car_make, inventory[i].car_model]);
  }
 }
console.log(oldCars.length);
//Stretch
console.log("Now with filter method!");

oldCars = [];
oldCars = carYears.filter((elem) => elem<2000);
console.log(oldCars.length);
...
BMWAndAudi = inventory.filter((elem) => (elem.car_make=='BMW' || elem.car_make=='Audi'));
console.log(JSON.stringify(BMWAndAudi));